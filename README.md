...
00-15 dk (Giriş): Yapay zeka nasıl çalışır? Claude API nedir? Gönderdiğimiz mesajlar Netlify üzerinden Anthropic sunucularına gidip nasıl geri geliyor? (Görselleştirerek anlatım).

15-35 dk (Uygulama): Netlify üzerindeki .js dosyasını oluşturma. system prompt kısmını beraber inceleyelim:

Görev: system içeriğini "TÜGVA İcathane Asistanı" olarak güncellemelerini isteyin. Öğrenciler kendi asistanlarına bir isim verebilirler.

35-45 dk (Kurulum): API Key'lerin sisteme tanımlanması ve fonksiyonun "Deploy" edilmesi.

Görev 1: Renkleri Değiştir (Root CSS)
Kodun en başındaki :root kısmını öğrencilerin TÜGVA renklerine göre güncellemelerini isteyelim:
CSS
:root {
  --bg: #f4f7f9;           /* Açık gri/mavi arka plan */
  --surface: #ffffff;      /* Beyaz yüzeyler */
  --accent: #00599c;       /* TÜGVA Mavisi */
  --accent2: #003366;      /* Koyu Lacivert */
  --text: #1a1a1a;         /* Koyu yazı */
  --border: #d1d9e6;       /* Yumuşak kenarlıklar */
  /* Diğer değişkenleri de buna göre uydurabilirler */
}
